...
export function OrganizationPicker({ value, onChange, filter = false, manageFolders = true, compact = false }: { value: OrganizationChoice; onChange: (value: OrganizationChoice) => void; filter?: boolean; manageFolders?: boolean; compact?: boolean }) {
  ...
  return <>
    <Pressable accessibilityRole="button" accessibilityLabel={filter ? 'Filter by folder or tag' : 'Choose folder and tags'} accessibilityValue={{ text: `${summary}${value.userTags.length ? ' · ' + value.userTags.join(', ') : ''}` }} onPress={() => setVisible(true)} style={({ pressed }) => [styles.trigger, compact && styles.compactTrigger, compact && (value.folderId !== undefined || value.userTags.length > 0) && styles.activeTrigger, pressed && { opacity: .7 }]}><Ionicons name="folder-outline" size={18} color={colors.accent} />{compact ? null : <><Text style={styles.triggerText} numberOfLines={1}>{summary}{value.userTags.length ? ` · ${value.userTags.map(tag => `#${tag}`).join(' ')}` : filter ? ' · filter' : ' · add tags'}</Text><Ionicons name="chevron-down" size={14} color={colors.muted} /></>}</Pressable>
...
const styles = StyleSheet.create({ compactTrigger: { width: 44, paddingHorizontal: 0, justifyContent: 'center', borderRadius: 9 }, activeTrigger: { backgroundColor: colors.accentSoft, borderColor: colors.accent }, trigger: { flexDirection: 'row', alignItems: 'center', gap: 8, minHeight: 44, paddingHorizontal: 12, backgroundColor: colors.surface, borderRadius: 10, borderWidth: 1, borderColor: colors.line }, triggerText: { color: colors.ink, fontSize: 13, flex: 1 }, toolbar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingHorizontal: 24, paddingTop: 8, paddingBottom: 8 }, done: { minWidth: 44, minHeight: 44, alignItems: 'center', justifyContent: 'center' }, doneText: { color: colors.accent, fontSize: 16, fontWeight: '600' }, page: { padding: 24, paddingBottom: 48, gap: 12 }, folderRow: { flexDirection: 'row', alignItems: 'center' }, folder: { flex: 1, minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.line }, folderName: { flex: 1, color: colors.ink, fontSize: 16 }, manage: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center' }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, chip: { minHeight: 44, paddingHorizontal: 13, paddingVertical: 10, borderRadius: 8, borderWidth: 1, borderColor: colors.line, justifyContent: 'center', backgroundColor: colors.surface }, selected: { backgroundColor: colors.ink, borderColor: colors.ink }, chipText: { color: colors.ink, fontSize: 13, flexShrink: 1 } });
