...
export function ProcessingControls(){
 ...
 return <FrostedPanel><Text style={typography.heading}>Processing, on your terms.</Text><Text style={typography.small}>New saves are organized after you enable this. You can turn it off anytime.</Text>
  ...
  <View style={{flexDirection:'row',flexWrap:'wrap',gap:8}}>{([{value:'instant',label:'Instant'},{value:'scheduled',label:'Scheduled'},{value:'manual',label:'Manual'},{value:'paused',label:'Paused'}] as const).map(mode=><Pressable key={mode.value} accessibilityRole="radio" accessibilityState={{checked:settings.mode===mode.value,disabled:busy}} disabled={busy} onPress={()=>void update({mode:mode.value})} style={{minHeight:44,padding:12,borderRadius:12,backgroundColor:settings.mode===mode.value?colors.accentSoft:colors.surface}}><Text style={typography.label}>{mode.label}</Text></Pressable>)}</View>
  ...
  {settings.mode==='scheduled'?<View style={{gap:8}}><Text style={typography.label}>Run every</Text><View style={{flexDirection:'row',gap:8}}>{([1,6,24] as const).map(hours=><Pressable key={hours} accessibilityRole="radio" accessibilityState={{checked:settings.intervalHours===hours}} disabled={busy} onPress={()=>void update({intervalHours:hours})} style={{minHeight:44,padding:12,borderRadius:12,backgroundColor:settings.intervalHours===hours?colors.accentSoft:colors.surface}}><Text style={typography.label}>{hours} {hours===1?'hour':'hours'}</Text></Pressable>)}</View>{settings.nextRunAt?<Text style={typography.small}>Next run: {new Date(settings.nextRunAt).toLocaleString()}</Text>:null}</View>:null}
