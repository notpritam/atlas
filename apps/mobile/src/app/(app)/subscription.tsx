...
export default function Subscription(){
  ...
  return <Screen top={false} bottom><ScrollView contentContainerStyle={styles.page}>
    ...
    {plan?.pro?<FrostedPanel><Text style={typography.heading}>{plan.complimentaryPro?'Pro beta — complimentary':'Your Pro plan is active'}</Text><Text style={typography.small}>{appStore?`${appStore.renews?'Renews':'Available until'} ${new Date(appStore.expiresAt).toLocaleDateString()}.`:web?'Your web subscription includes Pro in this app.':plan.complimentaryPro?'Your beta access includes Pro at no charge. No subscription is required.':'Pro is available on your FoundKeep account.'}</Text>{appStore?<Button secondary label="Manage App Store subscription" onPress={()=>void Linking.openURL('https://apps.apple.com/account/subscriptions')}/>:null}{web?<Text style={typography.small}>Manage your existing web subscription in your FoundKeep dashboard.</Text>:null}</FrostedPanel>:<FrostedPanel>
    ...
    <Button secondary label="Refresh plan" loading={loading} disabled={busy} onPress={()=>void refresh()}/>
