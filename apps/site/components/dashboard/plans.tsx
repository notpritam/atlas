...
export default function Plans() {
  ...
  return <><PageHeading title="Plans & usage" description="Start with a library you control. Add Pro when you want Foundkeep to process and organize your saves." />
    ...
    {!data ? <section className="plan-loading" role="status" aria-busy={plan.isPending}><h2>{plan.isError ? 'Your plan couldn’t load.' : 'Loading your plan…'}</h2><p className="muted">{plan.error?.message || 'Checking your subscription and collection limits.'}</p>{plan.isPending ? <ContentSkeleton kind="plan" /> : null}{plan.isError ? <button className="button secondary compact" onClick={() => void plan.refetch()}>Try again</button> : null}</section> : <div ref={content}>
      <section className="plan-overview" aria-labelledby="current-plan-title"><div className="plan-overview-heading"><div><h2 id="current-plan-title">Your {data.pro ? 'Pro' : 'Free'} plan</h2><p>{activeSubscription ? `${activeSubscription.renews ? 'Renews' : 'Access until'} ${date(activeSubscription.expiresAt)} · ${activeSubscription.provider === 'revenuecat' ? 'App Store' : 'Web subscription'}` : 'A private home for your collection, across every device.'}</p></div><button className="button secondary compact" disabled={busy} onClick={() => action.mutate('sync')}>{busy && action.variables === 'sync' ? <><Spinner />Refreshing…</> : 'Refresh plan'}</button></div>
        ...
        <div className="usage-grid">
          ...
          {usage ? <UsageMeter label="Processing credits" used={usage.used + usage.reserved} limit={data.limits.monthlyProcessing} value={data.pro ? `${number(usage.used)} / ${number(data.limits.monthlyProcessing)}` : 'Included with Pro'} help={data.pro ? `${number(usage.reserved)} queued · Resets each month (UTC).` : '500 monthly credits for text and image processing.'} /> : <div className="usage-meter"><h3>Processing credits</h3><p className="muted" role="status">{processing.isError ? 'Credit usage is unavailable.' : 'Loading credit usage…'}</p>{processing.isError ? <button className="subtle-button" onClick={() => void processing.refetch()}>Retry credit usage</button> : null}</div>}
        </div>
        <div className="plan-management">{data.billing.paddle.canManage ? <button className="text-link subtle-button" disabled={busy} onClick={() => action.mutate('portal')}>{busy && action.variables === 'portal' ? <><Spinner />Opening billing…</> : 'Manage subscription'}</button> : null}{data.subscriptions.some(subscription => subscription.provider === 'revenuecat') ? <a className="text-link" href="https://apps.apple.com/account/subscriptions" target="_blank" rel="noopener noreferrer">Manage in your Apple Account <span aria-hidden="true">↗</span></a> : null}{data.pro ? <Link className="text-link" href="/dashboard/settings/processing">Processing settings <span aria-hidden="true">→</span></Link> : null}</div>
      </section>
      {action.error ? <p className="form-message is-error" role="alert">{action.error.message}</p> : null}
      <section className="plan-options" aria-labelledby="compare-plans-title"><div className="plan-options-heading"><h2 id="compare-plans-title">Your agent on Free.<br />Foundkeep’s processing on Pro.</h2><p>Both plans give you control of your library. Choose who does the organizing.</p></div>
        <div className="plan-grid">
          <article className="plan-card" data-plan="free">
            ...
            <p className="plan-capacity"><strong>200 MB</strong> cloud storage <span aria-hidden="true">·</span> <strong>10,000</strong> saves</p>
            <Benefits items={[
              { title: 'Save and organize your everyday finds', description: 'Keep pages, images, highlights, and notes. Import bookmarks and organize them with folders and tags.' },
            ...
            <p className="plan-boundary">Foundkeep-managed image and link processing require Pro. Any costs for your own agent or model provider are separate.</p>
          ...
          <article className="plan-card pro-plan" data-plan="pro">
            <div className="plan-card-heading"><h3>Pro</h3>{data.pro ? <span className="current-plan-label">Current plan</span> : <span className="plan-extra-label">Foundkeep does the processing</span>}</div>
            ...
            <p className="plan-capacity"><strong>2 GB</strong> cloud storage <span aria-hidden="true">·</span> <strong>500</strong> processing credits / month</p>
      ...
      <section className="plan-roadmap" aria-labelledby="plan-roadmap-title">
        <div className="plan-roadmap-heading"><h2 id="plan-roadmap-title">More ways to keep video</h2><span className="plan-roadmap-label">Planned for Pro</span></div>
        <p className="plan-roadmap-description">These features are not available yet and are not included in the current Pro feature set.</p>
        <dl className="plan-media-list">
          <div><dt>YouTube</dt><dd>Video processing</dd><dd className="plan-media-status">Planned</dd></div>
          <div><dt>Instagram</dt><dd>Video downloads</dd><dd className="plan-media-status">Planned</dd></div>
          <div><dt>X / Twitter</dt><dd>Video downloads</dd><dd className="plan-media-status">Planned</dd></div>
        </dl>
        <p className="plan-roadmap-footnote">You can already save links to these platforms. Saving a link does not download or process the video.</p>
      </section>
      <section className="plan-details" aria-labelledby="plan-details-title"><h2 id="plan-details-title">Before you choose</h2><details><summary>Can my own agent make changes on Free?</summary><p>Yes. Both plans include MCP access. Give your agent organization permission to create folders, update tags, and connect related saves. Those changes are saved back to your Foundkeep library. You control its instructions and can revoke access in <Link className="text-link" href="/dashboard/agents">Agent connections</Link>. Your agent’s model and hosting costs are separate from Foundkeep.</p></details><details><summary>How do Pro processing credits work?</summary><p>Pro includes 500 managed processing credits per UTC calendar month. A successful processing job uses one credit. Queued jobs reserve credits; failed or cancelled jobs do not use them. Enable processing in Settings before any selected content is sent to OpenAI. Your own agent’s work does not use these credits unless it requests Foundkeep-managed processing.</p></details><details><summary>What happens if I return to Free?</summary><p>Your existing saves and own-agent access remain. Free includes 200 MB of cloud storage; new uploads may be limited if you’re over that allowance. Foundkeep-managed processing stops when your Pro access ends.</p></details></section>
    </div>}
  </>;
}
