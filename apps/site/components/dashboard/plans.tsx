...
export default function Plans() {
  ...
  return <><PageHeading title="Plans & usage" description={data?.complimentaryPro?"Complimentary Pro for the friends beta. All features, 2 GB of storage, and 500 monthly processing credits.":data?.earlyAccess?"All features are available during early access. Choose Pro for more cloud storage.":"Start with a library you control. Add Pro when you want FoundKeep to process and organize your saves."} />
    ...
    {!data ? <section className="plan-loading" role="status" aria-busy={plan.isPending}><h2>{plan.isError ? 'Your plan couldn’t load.' : 'Loading your plan…'}</h2><p className="muted">{plan.error?.message || 'Checking your subscription and collection limits.'}</p>{plan.isPending ? <ContentSkeleton kind="plan" /> : null}{plan.isError ? <button className="button secondary compact" onClick={() => void plan.refetch()}>Try again</button> : null}</section> : <div ref={content}>
      <section className="plan-overview" aria-labelledby="current-plan-title"><div className="plan-overview-heading"><div><h2 id="current-plan-title">{data.complimentaryPro ? 'Pro beta — complimentary' : `Your ${data.pro ? 'Pro' : 'Free'} plan`}</h2><p>{activeSubscription ? `${activeSubscription.renews ? 'Renews' : 'Access until'} ${date(activeSubscription.expiresAt)} · ${activeSubscription.provider === 'revenuecat' ? 'App Store' : 'Web subscription'}` : data.complimentaryPro ? 'Your beta access includes Pro at no charge. No subscription is required.' : 'A private home for your collection, across every device.'}</p></div><button className="button secondary compact" disabled={busy} onClick={() => action.mutate('sync')}>{busy && action.variables === 'sync' ? <><Spinner />Refreshing…</> : 'Refresh plan'}</button></div>
      ...
      <section className="plan-options" aria-labelledby="compare-plans-title"><div className="plan-options-heading"><h2 id="compare-plans-title">{data.earlyAccess?<>All features.<br />More room with Pro.</>:<>Your agent on Free.<br />FoundKeep’s processing on Pro.</>}</h2><p>{data.earlyAccess?"MCP, groups, and managed processing are open on every plan while we build FoundKeep together.":"Both plans give you control of your library. Choose who does the organizing."}</p></div>
        <div className="plan-grid">
          <PlanCard className="plan-card" data-plan="free">
            ...
            <div className="plan-card-action">{data.pro ? <p className="field-help">{data.complimentaryPro ? 'Your complimentary beta access includes the Pro allowance.' : 'To switch to Free, manage your subscription above. Your paid access continues until its expiry date.'}</p> : <span className="plan-current-state"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="m5 12 4 4L19 6" /></svg>You’re on Free</span>}<Link className="text-link plan-agent-link" href="/dashboard/agents">Connect your own agent <span aria-hidden="true">→</span></Link></div>
          </PlanCard>
          <PlanCard featured className="plan-card pro-plan" data-plan="pro">
            ...
            <p className="plan-price">{data.complimentaryPro ? 'Free' : price} <span>{data.complimentaryPro ? 'during beta' : '/ month'}</span></p><p className="plan-price-note">{data.complimentaryPro ? 'Complimentary Pro access. No payment required.' : `${data.price.currency} · Billed monthly. Cancel anytime.`}</p>
        ...
        <p className="plan-footnote">{data.complimentaryPro ? 'Complimentary Pro follows your FoundKeep account across all devices during the friends beta.' : `Web pricing is in ${data.price.currency}; applicable taxes are shown at checkout. App Store pricing is localized. Pro follows your FoundKeep account across all devices.`}</p>
      </section>
