...
import {CollectionArtwork,CollectionIcon,CuratorAvatar,PublicCollectionNav,PublicCollectionFooter,collectionIcons} from './public-chrome';
...
export function PublicCollection({initial,accountId,q='',tag=''}:{initial:CollectionDetail;accountId?:string;q?:string;tag?:string}) {
 ...
 const [view,setView]=useState<'list'|'grid'>('list'),[shareMessage,setShareMessage]=useState(''),[copyFallback,setCopyFallback]=useState(''),[searching,startSearch]=useTransition();
  ...
  <main ref={ref} className="public-collection">
   ...
   <header className="collection-profile">
    <div className="collection-profile-copy"><h1>{c.title}</h1>{c.description?<p className="collection-description">{c.description}</p>:null}
     <div className="collection-identity"><CuratorAvatar name={c.ownerName}/><div><span>Curated by <strong>{c.ownerName}</strong></span><small>{c.kind==='group'?'Group collection':'Personal collection'} · Updated {new Date(c.updatedAt).toLocaleDateString('en',{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'})}</small></div></div>
     <div className="collection-profile-meta"><span><CollectionIcon>{collectionIcons.bookmark}</CollectionIcon>{c.entries.toLocaleString('en-US')} {c.entries===1?'find':'finds'}</span><span>{c.followers.toLocaleString('en-US')} {c.followers===1?'follower':'followers'}</span><span><CollectionIcon>{c.visibility==='public'?collectionIcons.globe:collectionIcons.lock}</CollectionIcon>{c.visibility==='public'?'Public':'Private'}</span></div>
     <div className="collection-toolbar profile-actions">{accountId?<button className="button primary" disabled={busy} onClick={()=>void follow()}>{busy?<Spinner/>:<CollectionIcon>{c.following?collectionIcons.check:collectionIcons.plus}</CollectionIcon>}{c.following?'Following · unfollow':'Follow collection'}</button>:<Link className="button primary" href="/login"><CollectionIcon>{collectionIcons.plus}</CollectionIcon>Log in to follow</Link>}<button className="button secondary share-collection" onClick={()=>void share()}><CollectionIcon>{collectionIcons.share}</CollectionIcon>Share collection</button>{c.role?<Link className="text-link collection-manage-link" href={`/dashboard/collections/${c.id}`}>{c.canModerate?'Manage collection':'Open in my collections'}<CollectionIcon>{collectionIcons.arrow}</CollectionIcon></Link>:null}</div>
     {canSuggest?<div className="mobile-contribute-action">{c.canSubmit?<button className="text-link" aria-expanded={adding} onClick={()=>setAdding(!adding)}>{adding?'Close submission':c.requireApproval&&!c.canModerate?'Suggest a find':'Add a find'}<CollectionIcon>{collectionIcons.arrow}</CollectionIcon></button>:<Link className="text-link" href="/login">Log in to contribute<CollectionIcon>{collectionIcons.arrow}</CollectionIcon></Link>}<a href="#collection-context">Contribution rules</a></div>:null}
    ...
    </div><CollectionArtwork collection={c}/>
   </header>
   ...
   <div className="collection-reading-layout">
    <section className="collection-find-feed" id="collection-finds" aria-labelledby="collection-finds-title" data-view={view} aria-busy={searching}>
     <div className="collection-feed-heading"><h2 id="collection-finds-title">The collection <span>{c.entries}</span></h2><div className="collection-view-control" role="group" aria-label="Find layout"><button type="button" aria-label="List view" title="List view" aria-pressed={view==='list'} onClick={()=>setView('list')}><CollectionIcon>{collectionIcons.list}</CollectionIcon></button><button type="button" aria-label="Grid view" title="Grid view" aria-pressed={view==='grid'} onClick={()=>setView('grid')}><CollectionIcon>{collectionIcons.grid}</CollectionIcon></button></div></div>
     <form className="collection-find-search" action={basePath} onSubmit={event=>{event.preventDefault();const values=new FormData(event.currentTarget),params=new URLSearchParams();for(const name of ['q','tag']){const value=String(values.get(name)||'').trim();if(value)params.set(name,value);}startSearch(()=>router.push(basePath+(params.size?'?'+params:'')+'#collection-finds',{scroll:false}));}}><div className="find-search-input"><CollectionIcon>{collectionIcons.search}</CollectionIcon><label className="sr-only" htmlFor="find-search">Search this collection</label><input id="find-search" name="q" type="search" defaultValue={q} maxLength={100} placeholder="Search this collection…"/></div><label className="sr-only" htmlFor="find-topic">Filter by topic</label><select name="tag" id="find-topic" defaultValue={tag}><option value="">All topics</option>{topics.map(topic=><option key={topic} value={topic}>{topic}</option>)}</select><button type="submit" className="button secondary" disabled={searching}>{searching?<Spinner/>:'Search'}</button></form>
     <div className="collection-result-summary"><p role="status">{q||tag?`${total} ${total===1?'match':'matches'}${q?` for “${q}”`:''}${tag?` in ${tag}`:''}`:'Saved with a little context. Shared for a reason.'}</p>{q||tag?<Link href={basePath+'#collection-finds'} scroll={false}>Clear filters</Link>:<span>Newest first</span>}</div>
     ...
     {data.entries.length||(!q&&!tag)?<Entries collection={c} entries={data.entries} request={request} changed={()=>void refresh()} publicView/>:<div className="collection-no-matches"><CollectionIcon>{collectionIcons.search}</CollectionIcon><h2>No finds match yet.</h2><p>Try another word or explore all the finds in this collection.</p><Link className="button secondary" href={basePath+'#collection-finds'}>Clear filters</Link></div>}
    ...
    <aside id="collection-context" className="collection-context" aria-label="About this collection"><section className="collection-contribute"><CollectionIcon>{canSuggest?collectionIcons.plus:collectionIcons.bookmark}</CollectionIcon><h2>{canSuggest?'A good find belongs here.':'Picked with intention.'}</h2><p>{canSuggest?c.requireApproval?'Know something worth adding? Suggest a link or idea for the curator to review.':'Have something worth keeping? Add a link, a useful note, or an idea to this collection.':'Follow this collection to keep its curator’s finds close at hand.'}</p>{c.canSubmit?<button className="button primary" aria-expanded={adding} onClick={()=>setAdding(!adding)}>{adding?'Close submission':c.requireApproval&&!c.canModerate?'Suggest a find':'Add a find'}<CollectionIcon>{collectionIcons.arrow}</CollectionIcon></button>:!accountId&&c.submissionPolicy==='anyone'?<Link className="button primary" href="/login">Log in to contribute<CollectionIcon>{collectionIcons.arrow}</CollectionIcon></Link>:null}</section>
     <CollectionRules collection={c}/>{c.tags.length?<section className="collection-about-topics"><h2>Explore these topics</h2><div className="collection-tags">{c.tags.map(topic=><Link href={'/collections?q='+encodeURIComponent(topic)} key={topic}>{topic}</Link>)}</div></section>:null}<section className="collection-make-own"><h2>Your taste. Your collection.</h2><p>Keep it personal, open it to others, or share a little corner of the internet.</p><Link href="/dashboard/collections">Make a collection<CollectionIcon>{collectionIcons.arrow}</CollectionIcon></Link></section>
    </aside>
