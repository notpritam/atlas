...
export function PlanHighlights({ pro = false }: { pro?: boolean }) {
  return <dl className="plan-highlights" aria-label={`${pro ? 'Pro' : 'Free'} plan at a glance`}>
    ...
    <div><dt>Built-in processing</dt><dd>{pro ? <>500 credits<span>per month</span></> : <>Not included<span>Bring your own agent</span></>}</dd></div>
    <div><dt>Collections you can create</dt><dd>{pro ? 'Personal + groups' : 'Personal'}</dd></div>
  </dl>;
}
