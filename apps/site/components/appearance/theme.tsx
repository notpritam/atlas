...
const themeColors={light:'#f5fafc',dark:'#16191d'} as const;
...
export function ThemeProvider({children}:{children:ReactNode}){
 ...
 useEffect(()=>{
  ...
  const apply=()=>{const next=preference==='system'?(media.matches?'dark':'light'):preference;document.documentElement.dataset.theme=next;document.documentElement.style.colorScheme=next;document.querySelectorAll<HTMLMetaElement>('meta[name="theme-color"]').forEach(meta=>{meta.content=themeColors[next];});setResolved(next);};
...
export function ThemeControl({compact=false}:{compact?:boolean}){
 ...
 if(compact){const next=resolved==='light'?'dark':'light',label=`Switch to ${next} mode`;return <button className="theme-toggle" type="button" aria-label={label} title={label} onClick={()=>setPreference(next)}><svg aria-hidden="true" viewBox="0 0 24 24">{icons[next]}</svg></button>;}
