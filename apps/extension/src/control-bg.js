...
chrome.runtime.onMessage.addListener((msg, sender) => {
  if (msg?.k === "agent-toggle") {
    controllableTabId = msg.on ? (sender.tab?.id ?? null) : null;
    void sendStatus();
  }
});
chrome.tabs.onRemoved.addListener((tid) => { if (tid === controllableTabId) { controllableTabId = null; void sendStatus(); } });
