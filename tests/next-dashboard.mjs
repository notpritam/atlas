...
try {
  ...
  assert.equal(await page.locator('#capture-dialog[open]').count(), 1); assert.match(page.url(), /item=/);
  ...
  assert.equal(await page.locator('.capture-selected').count(), 1); assert.equal(await page.locator('#capture-dialog[open]').count(), 1); record('Stored screenshots and notes open in a rounded modal');
  ...
  await page.setViewportSize({ width: 390, height: 1000 }); await page.locator('.capture-note .capture-open').click(); await page.locator('#capture-dialog[open]').waitFor(); await page.locator('.expand-capture').click(); await page.waitForURL('**/dashboard/saved/**');
  assert.equal(await page.locator('.capture-reading-view').count(), 1); await page.screenshot({ path: path.join(evidence, 'reading-mobile.png'), fullPage: true }); record('Phone capture modal expands to the dedicated reading page');
