...
The follow-up submission kit is complete at `deploy/dist/foundkeep-cws-submission-kit-1.0.2.zip` (SHA-256 `fe687eb2e6a95bedb8bcd973269a8df011ed6c09a279a3875ccbe65364469bff`, 806,682 bytes), also copied into the final artifact directory. Current `deploy/STORE_LISTING.md`, `deploy/STORE_REVIEWER_GUIDE.md`, and `deploy/STORE_RELEASE_AUDIT.md` cover the native sidebar, automatic connection, detailed save fields, optional permissions and complimentary beta Pro. Fresh screenshots come from the exact Store ZIP in a disposable headed Chromium profile with synthetic local content. All 11 kit files match embedded checksums and a repeat build is byte-identical. The Store package tests passed 3/3.

Submission still needs publisher access. No authenticated control surface was established on the Linux host or enrolled Mac; only application/CLI metadata was inspected, never persisted browser profiles or cookies. API v2 needs securely supplied `CWS_PUBLISHER_ID`, `CWS_CLIENT_ID`, `CWS_CLIENT_SECRET`, and `CWS_REFRESH_TOKEN` authorized for the Chrome Web Store scope. Updating listing images/privacy answers/private reviewer instructions also requires the publisher Dashboard. Submit only with deferred publishing (`STAGED_PUBLISH` in v2); public Store rollout remains a later release action. This access blocker does not affect the prepared signed/manual packages.
