...
- [ ] Finish signed iOS/Android artifact distribution and installed-device checks. Signed production Android version5 is hosted with exact public hash and installed login/note/URL/PDF/download/logout checks passing. Fresh signed iOS21 includes mobile76e6ecf and is uploaded/AppleVALID/internal-ready. Android App Links propagation and actual external TestFlight access remain open. External review contact/tester access remains pending. Verify production origins and identities, then publish only verified TestFlight/Play test or APK links.
...
- [x] Implement and test the downloader boundary and runtime installation contract independently of production state. Reviewed at6e927bb; real MDN/Instagram/X bytes verified; tested YouTube source refuses upstream access. Runtime is not service-enabled until integration completes.
